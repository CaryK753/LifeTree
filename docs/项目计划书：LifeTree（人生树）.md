...
| 愿景能力 | 当前状态 | 关键缺口 |
| :--- | :--- | :--- |
| 长期跟踪信息 | ✅ 已落地 | events 表 + 半衰期 + 信源 cron 刷新（分钟级可配） + 双存储（PG 真相源 + Neo4j 镜像） |
| 基于知识图谱的信息收集与分析 | ✅ 已落地 | 7 类节点 + 7 类关系 + `PROPAGATE_RISK_FROM_EVENT` 多跳遍历 + 节点点击下钻原始事件 |
| 插件自定义信源 | ✅ 已落地 | AST 检查 + 契约验证 + 启停/删除/运行 + Docker 卷持久化；但属同进程 AST 黑名单，非真正安全沙箱 |
| AI 一站式问答（可调用所有服务/工具） | ✅ 已落地 | LangGraph ReAct + 14 工具 + MCP（HTTP/SSE/stdio）+ Skills + 100k 截断 + Tool Call 透明卡片 |
| 交叉验证 | 🟡 部分 | 有信源归因（`_enrich_with_sources` 按 credibility 排序选最佳源）+ Relationship 的 weight/confidence/source_id；**缺**显式跨源冲突检测、多源投票、冲突关系标注、源可信度随历史准确率自适应 |
...
| 成功率估算 | 🟡 部分（参数全硬编码） | 有 P10/P50/p90 + 风险可控度等级 + 归因瀑布；模型已从 noise-OR 改为加权几何准备度 + 相关性折减生存项。**但所有参数都是硬编码启发式常数**（见缺口 G），无任何数据支撑，输出看似精确实为伪精确；**缺**参数数据驱动化、真实结果回流、Brier Score 校准、分组漂移监控 |
| 信源自动发现/添加/评估 | 🔴 缺失 | 当前信源靠用户手动添加或 Tavily 关键词一次性搜索；**缺**基于目标上下文的 LLM 信源推荐、按领域可信度排序的候选源库、源历史准确率追踪与 credibility 自适应 |
| 风险领域自动感知/发现 | 🔴 缺失 | 风险因子为预置种子或手工添加，region 启发式筛选；**缺**从累积事件聚类发现新兴风险主题、LLM 抽取 risk theme 并提议纳入 RiskFactor、跨目标的风险传染可视化 |
| 规划阶段性任务/每日任务 | 🔴 缺失 | 仅有推理引擎输出的"无悔微行动"建议列表（`optimal_action_sequence`），无 Action 实体、无截止日、无日历、无每日任务面板、无进度回写；决策本体设计文档已规划 Action 实体但未落地 |
| 估算行为 ROI 和成功率 | 🔴 缺失 | 有情景级成功率，无行为级 ROI（成本 vs. 概率提升）、无成本-收益排序、无"最高杠杆动作"推荐 |
| 普通用户日常闭环 | 🔴 缺失 | 缺全局搜索、变更摘要（"自上次访问以来"）、行动日历/待办、整库备份恢复、统一错误态；首页主路径不唯一，侧栏入口过载 |

### 11.2 关键能力缺口详述

#### 缺口 A：信源自动发现与可信度自适应（Source Auto-Discovery）

**问题**：当前信源生命周期始于用户手动录入 URL 或 Tavily 一次性搜索。中长期决策的痛点恰在于"我不知道该盯哪里"。
...
**问题**：RiskFactor 目前是静态的——seed 脚本预置或用户手工建。真实世界中新兴风险（如"某省 PNP 暂停"、"汇率突破关键阈值"）应能被系统从事件流中自动识别并提议纳入跟踪。
...
**问题**：决策本体设计文档（`决策本体与预测模型设计`）已规划 `Action` 实体（责任人、截止日、执行状态、回写结果），但未落地。当前"无悔微行动"只是推理结果里的文本列表，无法追踪、无法回写、无法生成每日任务。
...
**问题**：当前成功率是情景级（P50），无法回答"我这周花 10 小时备考雅思，成功率提升多少？"。
...
**问题（最严重的可信度隐患）**：当前推理引擎的数学形式虽已从 noise-OR 修正为加权几何准备度 + 相关性折减生存项，但**所有参数都是硬编码的魔法常数，无任何数据支撑**：
...
后果：系统会输出 `P50=0.623` 这样看似精确的数字，但底层无数据支撑，是审计文档警告的"伪精确"。**在参数未数据驱动化之前，行为 ROI 估算（缺口 D）和自演化校准（缺口 F）都建立在沙堆上，不应向用户呈现为可信预测。**
...
| SQLite 本地桌面模式（personal + local，详见审计文档第 6 节） | 仓储适配器接口 | P3 |
...
- **P3（长线）**：SQLite 本地桌面模式、原生移动端。属产品形态扩展，不影响核心闭环。
...
领域扩展不需要改架构，只需新增场景模板（Goal + Pathway + Requirement + RiskFactor 种子数据）+ 对应信源推荐 prompt。已验证的领域模板方向：移民（FSW/OINP/PNP/CEC 已有种子）、留学、海外置业、职业转型、退休养老、数字游民规划。
